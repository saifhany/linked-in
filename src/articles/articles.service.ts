...
export class ArticlesService {
    ...
    async getFriendsArticles(uid: number): Promise<Article[]> {
        ...
        .orWhere(
            existsQuery(this.friendshipRepository.createQueryBuilder('F') // Friend's articles
    ...
    findArticleReaction(id: number) {
        return this.articleReactionsRepository.createQueryBuilder('R')
        .innerJoinAndSelect('R.article','article')
        .innerJoinAndSelect('R.reactor','reactor')
        .where('R.id = :id', { id: id }).getOne();
    }
    ...
    async addArticleReaction(articleId: number, reactorId: number, addArticleReactionDto: AddArticleReactionDto): Promise<ArticleReaction> {
        ...
        const r = await this.articleReactionsRepository.createQueryBuilder('R')
        .where('R.reactorId = :reactorId', { reactorId: reactorId })
        .andWhere('R.articleId = :articleId', { articleId: articleId })
        .getOne();
        if (r) {
