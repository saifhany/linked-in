...
export class UsersService implements OnModuleInit {
    ...
    async findSome(ids: number[]): Promise<User[]> {
        ...
        .leftJoinAndSelect('user.jobAlerts','jobAlert')
        .leftJoinAndSelect('user.educations','education')
        .leftJoinAndSelect('user.experiences','experience')
        .leftJoinAndSelect('user.articleReactions','reaction')
        .leftJoinAndSelect('reaction.article','r_article')
        .leftJoinAndSelect('user.articleComments','comment')
        .leftJoinAndSelect('comment.article','c_article')
        .leftJoinAndSelect('user.friendsAdded','addedFriend')
        .leftJoinAndSelect('addedFriend.user2', 'connection')
        .leftJoinAndSelect('user.friendsAccepted','acceptedFriend')
        .leftJoinAndSelect('acceptedFriend.user1', 'connection2')
        let result = await q.getMany();
        result.forEach(user => {
            let addedConnections = user.friendsAdded.map(f => f.user2);
            let acceptedConnections = user.friendsAccepted.map(f => f.user1);
            user.connections = Array.prototype.concat(addedConnections, acceptedConnections);
            delete user.friendsAdded;
            delete user.friendsAccepted;
        });
        return result;
    ...
    async changeProfilePic(uid: number, picName: string) {
        ...
        if (!ok) {      
            // The fs.unlinkSync() method is used to synchronously remove a file or symbolic 
    ...
    async getFriends(uid: number) {
        ...
        .andWhere(
        existsQuery( 
        this.friendshipsRepository.createQueryBuilder('F')
        .where('F.user1Id = :uid', { uid: uid })
        .andWhere('F.user2Id = U.id')
        .orWhere('F.user2Id = :uid', { uid: uid })
        .andWhere('F.user1Id = U.id')
        )
        )
        .leftJoinAndSelect('U.experiences','experience')
        .leftJoinAndSelect('experience.company','company')
    ...
    async removeSkillFromUser(uid: number, skillId: number) {
        const user = await this.findOne(uid);
        // splice(at postion ? , remove 1 item)
        // findIndex (number)
